...
void GamePlayScene5::UpdateCamera()
{
	//　カメラの演出
	if (isZoomIn_)
	{
...
void GamePlayScene5::UpdateZoomIn()
{
	// 時間を経過させる
...
void GamePlayScene5::UpdateZoomOut()
{
	// 時間を経過させる
	cameraEaseTime_ += 1.0f / 60.0f;
	float t = cameraEaseTime_ / cameraEaseDuration_;
...
void GamePlayScene5::CalculateNearestPosition()
{
	// プレイヤーが残り1人のとき
	if (players_.size() == 1)
	{
...
void GamePlayScene5::CheckShake() {
	//全てのプレイヤーのシェイク判定を処理
	for (auto& player : players_) {
